...
def fit_model(X, y, penalty, dual, tol, c,
                 fit_intercept, intercept_scaling, random_state,
                 solver, max_iter, multi_class, verbose,
                 warm_start, n_jobs, l1_ratio):   

    dual = True if dual == "True" else False   
    fit_intercept = True if fit_intercept == "True" else False
    warm_start = True if warm_start == "True" else False 
...
@callback([
          ...
          Input(f"logisticregresion", "n_clicks"), 
...
def display_page(n_clicks, options_y, value_y, data, test_size,
    ...
    if n_clicks:      
        ...
        try:         
            ...
            obj_middle = create_msg(f"""Modelo Entrenado.
